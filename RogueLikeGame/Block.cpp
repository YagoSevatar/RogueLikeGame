...
bool Block::IsBroken() { return hitCount <= 0; }
...
void SmoothDestroyableBlock::Update(float timeDelta) { UpdateTimer(timeDelta); }

bool SmoothDestroyableBlock::GetCollision(
    std::shared_ptr<Colladiable> collidableObject) const {
    if (isTimerStarted_) {
        return false;
    }
...
void SmoothDestroyableBlock::OnHit() { StartTimer(SETTINGS.BREAK_DELAY); }
