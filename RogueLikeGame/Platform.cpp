...
Platform::Platform(const sf::Vector2f& position)
    : GameObject(SETTINGS.TEXTURES_PATH + TEXTURE_ID + ".png", position,
                 (float)SETTINGS.PLATFORM_WIDTH,
                 (float)SETTINGS.PLATFORM_HEIGHT) {}
...
void Platform::Move(float speed) {
    ...
    position.x =
        std::clamp(position.x + speed, SETTINGS.PLATFORM_WIDTH / 2.f,
...
bool Platform::GetCollision(std::shared_ptr<Colladiable> collidable) const {
    ...
    if (ballPos.x < rect.left) {
        return sqr(ballPos.x - rect.left) + sqr(ballPos.y - rect.top) <
               sqr(SETTINGS.BALL_SIZE / 2.0);
    }

    if (ballPos.x > rect.left + rect.width) {
        return sqr(ballPos.x - rect.left - rect.width) +
                   sqr(ballPos.y - rect.top) <
               sqr(SETTINGS.BALL_SIZE / 2.0);
    }
...
bool Platform::CheckCollision(std::shared_ptr<Colladiable> collidable) {
    ...
    if (GetCollision(ball)) {
        auto rect = GetRect();
        auto ballPosInOlatform =
            (ball->GetPosition().x - (rect.left + rect.width / 2)) /
            (rect.width / 2);
