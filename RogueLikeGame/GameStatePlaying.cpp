...
void GameStatePlayingData::Init() {
    ...
    assert(
        gameOverSoundBuffer.loadFromFile(SETTINGS.SOUNDS_PATH + "Death.wav"));

    // factoriesInit
    factories.emplace(BlockType::Simple,
                      std::make_unique<SimpleBlockFactory>());
    factories.emplace(BlockType::ThreeHit,
                      std::make_unique<ThreeHitBlockFactory>());
    factories.emplace(BlockType::Unbreackable,
                      std::make_unique<UnbreackableBlockFactory>());

    // Init background
    background.setSize(
        sf::Vector2f(SETTINGS.SCREEN_WIDTH, SETTINGS.SCREEN_HEIGHT));
    ...
    auto platform = std::make_shared<Platform>(sf::Vector2f(
    ...
    auto ball = std::make_shared<Ball>(
        sf::Vector2f({SETTINGS.SCREEN_WIDTH / 2.f,
                      SETTINGS.SCREEN_HEIGHT - SETTINGS.PLATFORM_HEIGHT -
                          SETTINGS.BALL_SIZE / 2.f}));
    ...
    bonuses.emplace(BonusType::BiggerPlatform,
                    Bonus(
...
void GameStatePlayingData::Update(float timeDelta) {
    static auto updateFunctor = [timeDelta](auto obj) {
    ...
    std::shared_ptr<Platform> platform =
        std::dynamic_pointer_cast<Platform>(gameObjects[0]);
    std::shared_ptr<Ball> ball =
        std::dynamic_pointer_cast<Ball>(gameObjects[1]);
    ...
    blocks.erase(
        std::remove_if(blocks.begin(), blocks.end(),
                       [ball, &hasBrokeOneBlock, &needInverseDirX,
                        &needInverseDirY, this](auto block) {
                           if ((!hasBrokeOneBlock) &&
                               ...
                               GetBallInverse(ballPos, blockRect,
                                              needInverseDirX, needInverseDirY);
                           }
...
void GameStatePlayingData::createBlocks() {
    ...
    for (auto pairPosBlockTYpe : level.m_blocks) {
        ...
        sf::Vector2f position{
            (float)(SETTINGS.BLOCK_SHIFT + SETTINGS.BLOCK_WIDTH / 2.f +
                    pos.x * (SETTINGS.BLOCK_WIDTH + SETTINGS.BLOCK_SHIFT)),
...
void GameStatePlayingData::GetBallInverse(const sf::Vector2f& ballPos,
                                          const sf::FloatRect& blockRect,
                                          bool& needInverseDirX,
    ...
    if (auto block = std::dynamic_pointer_cast<Block>(observable); block) {
        ...
        if (breackableBlocksCount == 0) {
            ...
            if (SETTINGS.BONUS_PROPABILITY_PERCENT >= percent) {
                BonusType bonusType =
                    (BonusType)(random<int>(0, (int)BonusType::Count - 1));
