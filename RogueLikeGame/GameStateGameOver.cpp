...
void GameStateGameOverData::Init() {
    ...
    if (!isPlayerInTable) {
        ...
        sstream << SETTINGS.MAX_RECORDS_TABLE_SIZE << ". "
                << SETTINGS.PLAYER_NAME << ": " << playerScores;
        text.setString(sstream.str());
        text.setFillColor(sf::Color::Green);
    }
...
void GameStateGameOverData::Update(float timeDelta) {
    ...
    sf::Color gameOverTextColor =
        (int)timeSinceGameOver % 2 ? sf::Color::Red : sf::Color::Yellow;
...
void GameStateGameOverData::Draw(sf::RenderWindow& window) {
    ...
    // We need to create new vector here as DrawItemsList needs vector of
    // pointers
